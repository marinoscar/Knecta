...
interface AgentConfigSettingsProps {
  agentKey: 'dataAgent' | 'semanticModel';
  agentDisplayName: string;
  settings: SystemSettings;
  onSave: (agentConfigs: Record<string, AgentProviderConfig>) => Promise<void>;
  disabled?: boolean;
}
...
export function AgentConfigSettings({
  ...
  const [providerConfigs, setProviderConfigs] = useState<Record<string, AgentProviderConfig>>(
    settings.agentConfigs?.[agentKey] || {},
  );
  ...
  useEffect(() => {
    ...
    setProviderConfigs(settings.agentConfigs?.[agentKey] || {});
  }, [settings, agentKey]);
  ...
  const hasChanges = (): boolean => {
    const original = settings.agentConfigs?.[agentKey] || {};
          ...
          <Select
            ...
            onChange={(e) => {
              ...
              updateProviderConfig(providerType, {
                ...
                customBudget:
                  newLevel === 'custom' ? (config.customBudget || 1024) : undefined,
              });
            }}
            disabled={disabled}
          >
        ...
        {providerType === 'anthropic' && isCustomBudget && (
          <TextField
            ...
            onChange={(e) => {
              ...
              if (!isNaN(value)) {
                updateProviderConfig(providerType, {
                  customBudget: Math.max(1024, Math.min(128000, value)),
                });
              }
            }}
            disabled={disabled}
      ...
      <Paper key={providerType} sx={{ p: 3, mb: 3 }}>
        ...
        <Box sx={{ mb: 3 }}>
          <Typography gutterBottom>
            Temperature:{' '}
            {config.temperature !== undefined ? config.temperature.toFixed(1) : '0.0'}
          </Typography>
          <Slider
            value={config.temperature !== undefined ? config.temperature : 0}
            onChange={(_, value) =>
              updateProviderConfig(providerType, { temperature: value as number })
            }
            min={0}
            max={2}
            step={0.1}
  ...
  if (providers.length === 0) {
    return (
      <Alert severity="info">
        No LLM providers are configured. Add providers in the &quot;LLM Providers&quot; tab first.
      </Alert>
    );
  }
  ...
  return (
    <Box>
      <Typography variant="h6" gutterBottom>
        {agentDisplayName} Configuration
      </Typography>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Configure per-provider LLM parameters for {agentDisplayName}. Only enabled providers are
        shown.
      </Typography>
